...
export default function ServerList({ servers, loading, onServerDeleted, onServerUpdated }: ServerListProps) {
  ...
  const handleEdit = (server: IServer) => {
    ...
    setEditForm({
      name: server.name,
      monthlyCostUSD: server.monthlyCostUSD,
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  {editingServer?._id === server._id ? (
                    ...
                    <div className="flex items-center">
                      ...
                      {server.isBaseServer && (
                        <span className="ml-2 px-2 py-1 text-xs font-semibold text-green-800 dark:text-green-200 bg-green-100 dark:bg-green-800 rounded-full">
                          Base Server
                        </span>
                      )}
                    </div>
                      ...
                      {editingServer?._id === server._id ? (
                        <>
                          <button
                            ...
                            className="p-1 text-green-600 hover:text-green-800 dark:text-green-400 dark:hover:text-green-300 transition-colors duration-150"
                            title="Save"
                          >
